...
fasta_data = {}
classes = ['Tibetan', 'Andes', 'Ethiopia']
for curr_class in classes:
    f = open(path + '/Data/world/' + curr_class + '.fasta', 'r')
    curr_subject = ''
    for line in f:
        if line.startswith('>'):
...
for i in range(0, len(rcrs_dict['orig'])):
    ...
    if orig_id == rcrs_id:
        for key in fasta_data:
            if len(data[key]) < orig_id:
                data[key] += fasta_data[key][orig_id - 1]
    elif i > 0:
...
writer = pd.ExcelWriter(path + '/Data/world/rcrs/correspond.xlsx', engine='openpyxl')
...
for curr_class in classes:
    f = open(path + '/Data/world/rcrs/wo_hg/' + curr_class + '.fasta', 'w')
    curr_ids = [i for i, x in enumerate(subjects_dict['group']) if x == curr_class]
    for curr_id in curr_ids:
        f.write('>' + subjects_dict['subject'][curr_id] + '\n')
